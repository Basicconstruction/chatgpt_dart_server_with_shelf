...
Future<Response> chatHandle(Request request)async{
  ...
  if(key.length!=13||keyI==null){
    return Response(201,body:'key格式不符合规则，该key是用来辨认客户身份的key。');
  }
  if(RequestHandler.containRequestKey(key)){
    return Response(202,body:'请等待先前的请求结束');
  }
  KeyRequestTimes.addRequestTime(key);//标识用户请求进入用户请求队列
  final apiKey = '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';
  ...
  if(av_token<100){
    return Response(203,body: '剩余token数较少，无法得到有效的回答，请充值token');
  }
  ...
  if (response.statusCode == 200) {
    ...
    int consume = int.tryParse(data['usage']['total_tokens'].toString())??0;
    KeyHandler.updateByConsume(key, consume);
    //
    return Response.ok(completions);
  }
  return Response(500,body:'服务器错误,请联系管理员或站长');
